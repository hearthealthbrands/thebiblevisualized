document.addEventListener('DOMContentLoaded', () => {
    ...
    images.forEach(img => {
        if (img.complete) {
            ...
            img.addEventListener('error', () => {
                ...
                revealImage(img); // Show it anyway so we see the broken link icon instead of nothing
            });
        }
    });

    // FAILSAFE: Force all images to show after 0.5s (Fixes "Invisible Image" bug)
    setTimeout(() => {
    ...
    if (copyBtn && verseTextElement) {
        ...
        copyBtn.addEventListener('click', async () => {
            try {
                ...
                // Create Heart using Unicode to avoid encoding errors
    ...
    document.addEventListener('touchend', (e) => {
        if (touchStartX - e.changedTouches[0].screenX > 50 && nextUrl) navigate(nextUrl);
        if (e.changedTouches[0].screenX - touchStartX > 50 && prevUrl) navigate(prevUrl);
    }, false);
    ...
    if (sharePlaceholder) {
        ...
        // Safely defined icons to prevent line-break errors
        const iconX = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M13.6823 10.6218L20.2391 3H18.6854L12.9921 9.61788L8.44486 3H3.2002L10.0765 13.0074L3.2002 21H4.75404L10.7663 14.0113L15.5685 21H20.8131L13.6819 10.6218H13.6823ZM11.5541 13.0956L10.8574 12.0991L5.31391 4.16971H7.70053L12.1742 10.5689L12.8709 11.5655L18.6861 19.8835H16.2995L11.5541 13.096V13.0956Z"/></svg>';
        const iconFB = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"/></svg>';
        const iconPin = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.199.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.399.165-1.495-.69-2.433-2.852-2.433-4.587 0-3.728 2.705-7.149 7.833-7.149 4.105 0 7.292 2.934 7.292 6.852 0 4.089-2.578 7.38-6.16 7.38-1.205 0-2.344-.629-2.735-1.371 0 0-.599 2.289-.744 2.852-.271 1.026-1.002 2.302-1.492 3.111 1.118.327 2.298.502 3.523.502 6.627 0 12-5.368 12-11.987C24.017 5.367 18.647 0 12.017 0z"/></svg>';
        ...
        const html = `
            <div class="share-bar">
                ...
                <a href="https://twitter.com/intent/tweet?text=${title}&url=${url}" target="_blank" class="share-icon" aria-label="Share on X">${iconX}</a>
                <a href="https://www.facebook.com/sharer/sharer.php?u=${url}" target="_blank" class="share-icon" aria-label="Share on Facebook">${iconFB}</a>
                <a href="https://pinterest.com/pin/create/button/?url=${url}&media=${media}&description=${title}" target="_blank" class="share-icon" aria-label="Share on Pinterest">${iconPin}</a>
                <button id="copy-link-btn" class="share-icon" aria-label="Copy Link">${iconLink}</button>
            </div>
        `;
